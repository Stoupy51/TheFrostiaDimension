...
#Others
	...
	execute if score #santa_claus frostia.data matches 1.. as @e[type=zombie,tag=frostia.santa_claus] at @s run function frostia:boss/tick
	execute if score #santa_claus frostia.data matches 1.. as @e[type=marker,tag=frostia.santa_claus,predicate=!frostia:has_vehicle] at @s run function frostia:boss/kill_marker
